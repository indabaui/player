...
function createCallbacks(self) {
  return {
    ...
    whileplaying: function() {
      ...
      self.emit('playing', { position: this.position });
    },
    onfinish: function() {
      self.updateUi();
      self.emit('finish');
...
Player.prototype.updatePositionUi = function() {
  this.position.text(formatMs(this.sound.position) + ' / ' + formatMs(this.sound.duration));
